  ...
  return (
    <div className="scrollable" style={{flex:1,background:'var(--bg)',color:'var(--text)'}}>
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'20px 20px 12px'}}>
        <div>
          <div style={{fontSize:13,color:'var(--text2)'}}>{t('greeting')}</div>
        ...
        <div style={{display:'flex',gap:10,alignItems:'center'}}>
          ...
          <Link href="/profile" style={{width:40,height:40,borderRadius:'50%',background:'linear-gradient(135deg,var(--gold3),var(--gold))',display:'flex',alignItems:'center',justifyContent:'center',fontSize:20,textDecoration:'none'}}>
      ...
      <div style={{padding:'0 20px',marginBottom:16}}>
        <div style={{borderRadius:20,padding:20,background:'linear-gradient(135deg,rgba(200,150,0,0.12),rgba(232,197,106,0.06))',border:'1px solid rgba(200,150,0,0.25)'}}>
          ...
          <div style={{fontSize:12,color:'var(--text2)',display:'flex',justifyContent:'space-between',marginBottom:6}}>
            <span>XP</span><span>{profile.xp||0} / {xpNext}</span>
          </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'0 20px',marginBottom:16}}>
        <Link href="/attendance" style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,padding:16,textDecoration:'none',color:'var(--text)'}}>
          <div style={{fontSize:20,marginBottom:6}}>📅</div>
          <div style={{fontSize:24,fontWeight:800}}>{profile.events_attended||0}</div>
          <div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{t('eventsAttended')}</div>
        </Link>
        <Link href="/community" style={{background:'var(--card)',border:'1px solid var(--border)',borderRadius:16,padding:16,textDecoration:'none',color:'var(--text)'}}>
          <div style={{fontSize:20,marginBottom:6}}>🔥</div>
          <div style={{fontSize:24,fontWeight:800}}>{profile.streak_current||0}</div>
          <div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{t('dayStreak')}</div>
        </Link>
      </div>
      ...
      <div style={{padding:'0 20px',marginBottom:20}}>
        <div style={{borderRadius:16,padding:16,display:'flex',alignItems:'center',gap:16,background:'linear-gradient(135deg,rgba(255,124,58,0.12),rgba(200,165,60,0.06))',border:'1px solid rgba(255,124,58,0.25)'}}>
          ...
          <div style={{flex:1}}>
            <div style={{fontSize:20,fontWeight:800,color:'var(--orange)'}}>{profile.streak_current||0} {t('dayStreak')}</div>
            <div style={{fontSize:12,color:'var(--text2)',marginTop:2}}>{t('best')}: {profile.streak_max||0} {t('days')}</div>
